...
import { SchemaDefinition, TableDefinition, ColumnDefinition, RelationshipDefinition } from '../llm/gemini-client.js';
...
export class SchemaParser {
  async parse(input: string, options: ParseOptions = {}): Promise<SchemaDefinition> {
  ...
  private detectFormat(input: string): SchemaFormat {
  ...
  private parseSQLDDL(ddl: string): SchemaDefinition {
  ...
  private parseJSON(jsonStr: string): SchemaDefinition {
    try {
      const data = JSON.parse(jsonStr);
      
      // If it's already in our schema format
      if (data.tables && Array.isArray(data.tables)) {
        return data as SchemaDefinition;
      }
  ...
  private parseJSONTableInfo(tableInfo: any): ColumnDefinition[] {
    const columns: ColumnDefinition[] = [];
    ...
    // Handle object with column definitions
      ...
      for (const [colName, colInfo] of Object.entries(tableInfo)) {
        if (typeof colInfo === 'string') {
          ...
          columns.push({
            name: colName,
            type: (colInfo as any).type || 'VARCHAR(255)',
            nullable: (colInfo as any).nullable !== false,
            defaultValue: (colInfo as any).default,
  ...
  private parseCSV(csvStr: string): SchemaDefinition {
  ...
  private async parseNaturalLanguage(description: string): Promise<SchemaDefinition> {
    try {
...
Return a JSON object with this structure:
{
  ...
  private basicNaturalLanguageParse(description: string): SchemaDefinition {
    const tables: TableDefinition[] = [];
  ...
  private inferSchemaFromData(data: Record<string, unknown>[]): SchemaDefinition {
    if (data.length === 0) {
      return { tables: [] };
    }
  ...
  private inferTypeFromValue(value: unknown, allValues: unknown[]): string {
    ...
    if (nonNullValues.every(v => typeof v === 'number' && Number.isInteger(v))) {
      const max = Math.max(...(nonNullValues as number[]));
      const min = Math.min(...(nonNullValues as number[]));
    ...
    if (nonNullValues.every(v => {
      ...
      const sample = nonNullValues[0] as string;
  ...
  private inferTypeFromSamples(lines: string[], columnIndex: number): string {
    const values = lines
  ...
  private inferRelationships(schema: SchemaDefinition): RelationshipDefinition[] {
    const relationships: RelationshipDefinition[] = [];
  ...
  private detectOptimalIndexes(schema: SchemaDefinition): void {
    for (const table of schema.tables) {
      const indexes = [];
  ...
  private enrichWithSampleData(schema: SchemaDefinition, sampleData: Record<string, unknown>[]): void {
    ...
    for (const table of schema.tables) {
      for (const column of table.columns) {
        ...
        if (values.length > 0) {
          (column as any).examples = values.slice(0, 3);
          (column as any).distinctCount = new Set(values).size;
          
          if (typeof values[0] === 'number') {
            (column as any).min = Math.min(...(values as number[]));
            (column as any).max = Math.max(...(values as number[]));
          }
        }
      }
    }
  ...
  private extractDefault(constraints: string): unknown {
  ...
  private extractConstraints(constraintStr: string): string[] {
