...
export class QueryExecutor {
  ...
  constructor(private databasePath: string = ':memory:') {}
  
  async execute(sql: string, options: ExecutionOptions = {}): Promise<ExecutionResult> {
  ...
  private validateSQL(sql: string, readonly: boolean = false): void {
  ...
  private async createSchema(schema: any): Promise<void> {
    if (!this.db) return;
  ...
  private mapDataType(type: string): string {
    ...
    if (upper.includes('DECIMAL') || upper.includes('NUMERIC') || upper.includes('FLOAT') || upper.includes('DOUBLE')) return 'REAL';
  ...
  private insertSampleData(tableName: string, data: any[]): void {
...
export class MockQueryExecutor extends QueryExecutor {
  override async execute(sql: string, options: ExecutionOptions = {}): Promise<ExecutionResult> {
