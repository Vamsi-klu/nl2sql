...
import { SimpleFileProcessor } from './file-processing/simple-processor.js';
...
app.post('/api/upload', upload.array('files', 4), async (req, res) => {
    try {
        ...
        for (const file of req.files) {
            try {
                const processedFile = await fileProcessor.processFile(file.path, file.originalname, file.mimetype);
