...
import { initializeOpenAIClient, SQLDialect } from '../llm/openai-client.js';
    ...
    try {
        ...
        if (typeof validatedData.schema === 'string') {
            ...
            parsedSchema = await parser.parse(validatedData.schema, {
                format: validatedData.schemaFormat || SchemaFormat.AUTO_DETECT,
        ...
        const request = {
            ...
            schema: parsedSchema,
            dialect: validatedData.dialect || SQLDialect.POSTGRESQL,
...
app.post('/api/validate', async (req, res, next) => {
    try {
        ...
        const isValid = await openAIClient.validateSQL(sql, dialect || SQLDialect.POSTGRESQL);
...
app.post('/api/detect-schema', async (req, res, next) => {
    try {
        const { data } = req.body;
